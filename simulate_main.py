...
def get_past30_data(tickers):
    ...
    def save_30_data(ticker):
        ...
        start_time = end_time.replace(hour=10, minute=0, second=0, microsecond=0)
        if SIMULATION_MODE:
            prices_1 = api.get_trades(str(ticker), start=start_time.isoformat(), end=(start_time + timedelta(minutes=1, seconds=30)).isoformat(), limit=10000).df[['price']]
            prices_2 = api.get_trades(str(ticker), start=(end_time - timedelta(minutes=1, seconds=30)).isoformat(), end=end_time.isoformat(), limit=10000).df[['price']]
        else:
            prices_1 = api.get_trades(str(ticker), start=start_time.isoformat(), end=(start_time + timedelta(minutes=1, seconds=30)).isoformat(), limit=10000).df[['price']]
            prices_2 = api.get_trades(str(ticker), start=(end_time - timedelta(minutes=1, seconds=30)).isoformat(), end=end_time.isoformat(), limit=10000).df[['price']]
        ...
        if SIMULATION_MODE:
            quotes_1 = api.get_quotes(str(ticker), start=start_time.isoformat(), end=(start_time + timedelta(minutes=1, seconds=30)).isoformat(), limit=10000).df[['ask_price']]
            quotes_2 = api.get_quotes(str(ticker), start=(end_time - timedelta(minutes=1, seconds=30)).isoformat(), end=end_time.isoformat(), limit=10000).df[['ask_price']]
        else:
            quotes_1 = api.get_quotes(str(ticker), start=start_time.isoformat(), end=(start_time + timedelta(minutes=1, seconds=30)).isoformat(), limit=10000).df[['ask_price']]
            quotes_2 = api.get_quotes(str(ticker), start=(end_time - timedelta(minutes=1, seconds=30)).isoformat(), end=end_time.isoformat(), limit=10000).df[['ask_price']]
...
def return_ROC_list(tickers, timeframe):
    ...
    for i in range(len(tickers)):
        ...
        ROC_tickers.append(ROC(df['ask_price'], timeframe)) # [-1] for last value (latest)
    return ROC_tickers
...
def main():
    ...
    if SIMULATION_MODE or api.get_clock().is_open == True:
    ...
    while True:
        ...
        tickers = TICKERS
        try:
            # Ensure the correct import statement is present
            if SIMULATION_MODE or api.get_clock().is_open == True:
                # Check if we have made the first ever trade yet
                if os.path.isfile('FirstTrade.csv'):
                    # First trade has been made, set timeframe to 1 minute
                    ...
                    # First trade has not been made, set trade time to 10:00 AM yesterday
                    yesterday = datetime.now() - timedelta(days=1)
                    trade_time = yesterday.replace(hour=10, minute=0, second=0, microsecond=0)

                    '''
                    if ((datetime.now().astimezone(timezone('America/New_York')))).strftime('%H:%M:%S') < '10:00:00':
                        time_to_10 = int(str(datetime.strptime('10:00:00', '%H:%M:%S') - datetime.strptime(((datetime.now().astimezone(timezone('America/New_York')))).strftime('%H:%M:%S'), '%H:%M:%S')).split(':')[1]) * 60 + int(str(datetime.strptime('10:00:00', '%H:%M:%S') - datetime.strptime(((datetime.now().astimezone(timezone('America/New_York')))).strftime('%H:%M:%S'), '%H:%M:%S')).split(':')[2])
                    ...
                    df.to_csv('FirstTrade.csv', index=False)
            else:
