...
message =  "This project is intended for users with some knowledge in Finance and stats. Please read the documentation to understand. Please Read the documentation to understand what each platform does technically"
st.warning(message)
global start_date
global end_date
...
def dates():
    ...
    with st.form(key='start_end_dates'):
        ...
        if st.form_submit_button("Submit"):
             st.success("Recorded the start and end dates. Proceed to the strategies") 
def carry_out_strategies():
    #Add the Lumibot strategies
    with st.form(key="strategies"):
        st.header("Backtesting and Trading Strategies using Lumibots API")
        option = st.radio(
